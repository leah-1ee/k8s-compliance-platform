...
| Seconds Since Last Gatekeeper Audit | `time() - gatekeeper_audit_last_run_time` | Gatekeeper의 마지막 감사 실행 이후 경과 시간을 초 단위로 표시한다. |
| Gatekeeper Violations by Enforcement | `sum by (enforcement_action) (gatekeeper_violations)` | Gatekeeper 정책 위반을 enforcement action 기준으로 집계한다. |
| Policy Violations by Policy | `sum by (name) (gatekeeper_violations)` | Gatekeeper 정책별 위반 수를 집계한다. |
